...
def main():
    ...
    for i in range(0, 1048574, 104858):  # 64K Memory
        t.delete_row(7)
        if i < 30000:
            t.gen_text(
    ...
    t.gen_text(
        bottom_text,
        contin=True,
        row_num=int(t.num_rows) - 3,
        col_num=((int(t.num_cols) - len(bottom_text)) // 2),
    )
    for i in range(0, 30):
        for j in range(2, 22):
            t.delete_row(j)
    ...
    line_with_melbourne = (
        line_with_melbourne[:32]
        + "\x1b[32m"
        + "(O)"
        + "\x1b[0m"
        + line_with_melbourne[35:]
    )
    ...
    t.gen_text(
        pword_prmpt, row_num=int(t.num_rows) - 1, col_num=20, contin=True, count=10
    )
    t.toggle_show_cursor(True)
    # t.toggle_blink_cursor(False)
    t.gen_typing_text(uname, int(t.num_rows) - 2, contin=True, speed=0)
    t.gen_typing_text(pword, int(t.num_rows) - 1, contin=True, speed=0)
    t.clone_frame(5)
    ...
    t.gen_text(
        "\x1b[91mchm\x1b[0m\x1b[90mod +x README.sh && ./README.sh\x1b[0m",
        1,
        contin=True,
    )
    t.clone_frame(10)
    t.delete_row(1, prompt_col)
    t.gen_text(
        "\x1b[32mchmod\x1b[0m +x README.sh && ./README.sh\x1b[0m", 1, contin=True
    )
    t.clone_frame(5)
